...
ADT (Android Debug Tools) 是一个 Python CLI 工具集，用于 Android 调试和探索。基于 Click 构建 CLI，使用 Rich 美化终端输出，依赖 `adb`（需在 PATH 中）。
...
- 所有命令都是顶层命令（扁平化设计），无命令组。例如：`adt current`、`adt maps`、`adt backup`。
